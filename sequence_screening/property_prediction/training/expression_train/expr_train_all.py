# train the model used for predict_expr_emb_main_pickle.py
...
if __name__ == '__main__':
    ...
    res_dir = os.path.join(model_root, "property_model/expr/result_{}_a{}_lr{}_regk{}_f1_prop_".format(BATCH_SIZE,cls_alpha,lr,REG_LOSS_K))
    ...
    train_feats=np.concatenate([feats1,feats2],axis=0)
    train_reg_label=np.concatenate([label1,label2],axis=0)
    
    del feats1,feats2,label1,label2

    train_cls_label=(train_reg_label>=1).reshape(-1,1)
